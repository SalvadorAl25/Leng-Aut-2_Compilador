...
public class GeneradorCodigo 
{
	...
	String procesos[]=new String [4]; //Separar cada uno de los procesos que tiene acción
	int inicio;
	...
	{
		//Int.imprimirConsola("Generador de Código");
		for (inicio = 0; inicio < secuencias.size(); inicio++) 
		{
			...
			if(accion.startsWith("Comparacion"))
			{
				...
				else
				{
					int jaja=pasos.size()-1;
					String xD=pasos.elementAt(jaja).toString();
					//No se cumplió la condición, igual buscaremos si es if o while
					if(xD.startsWith("etq if"))
					{
						datoactual="else";
						...
						for (int j = 0; j < TVariables.length; j++) 
						{
							if(procesos[1].equals(TVariables[j][2]))
							{
								...
								{	//No hay necesidad de buscar valores
									TVariables[j][3]=EvaluaciónPosfija(procesos[2], TVariables[j][1]);
								}
								else
								{  //Tenemos variables
			...
			{
				//Terminará hasta que encuentre un end
				if(omitir_else)
				{
					omitir_else=false;
			...
			{
				//Eliminará todo lo que tenga entre el begin y el end
	...
	public boolean resComparaciones(String simbolo, String v1, String v2)
	{
		...
		if(datosiguales)
		{
			...
			if(tipodato.equals("char"))
			{
				...
				{
					System.out.println("Las variables char no pueden son valores numéricos, así que solo se pueden comparar con el símbolo ==");
				}
			}
			//String
			if(tipodato.equals("String"))
			{
				...
				{
					System.out.println("Las variables String no pueden son valores numéricos, así que solo se pueden comparar con el símbolo ==");
				}
			}
			return false;
		}
	...
	public void AsignarVariables(String TVariables[][], int j)
	{
		...
		TVariables[j][3]=EvaluaciónPosfija(jaja, TVariables[j][1]);
	}
	...
	public String EvaluaciónPosfija(String expresion, String tipodato)
	{
