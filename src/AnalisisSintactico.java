...
public class AnalisisSintactico {
	...
	{
		//Int.imprimirConsola("Analizador Sintáctico");
		...
		if(archivodatos.size()>0)
		{
			...
			int punterocad=0, punteropila=pila.size()-1, x=0, y=0, cad=archivodatos.size();  //x posición de terminales, y posición de NT
				...
				for(int i=0; i<=cad; i++)
				{
					try
					{
						...
						while(siguiente==false) {
							//Buscará si el primer elemento de la pila es un terminal o no
					...
					{
						System.out.println("Término de cadena.");
						terminado=true;
						break;
					}
			...
			if(valido)
			{
				System.out.println("Cadena válida para el autómata");
				AnalizadorSemantico as=new AnalizadorSemantico(cv, cc, Int);
				as.Analizar(archivoriginal, archivodatos, Int);
			}
			else
				System.out.println("Cadena no válida para el autómata");
