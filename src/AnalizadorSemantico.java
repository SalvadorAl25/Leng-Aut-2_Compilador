...
public class AnalizadorSemantico 
{
	...
	String tipodato[]={"int","double", "char", "String"}, datoactual=""; //El datoactual es para la etiqueta que está en ese momento
	...
	String datos[]={"Posición", "TipoDato", "NombreVariable", "Valor", "NivelIf", "DentroElse(True o false)", "NivelDoWhile", "NivelBeginEnd"};
	String TablaVariables[][], TablaCondiciones[][];
	
	public void Analizar(Vector archoriginal, Vector archivodatos, Interfaz Int)
	{
		//Int.imprimirConsola("Analizador Semántico");
		String impmensj="";
		variables=v+1;
		...
		if(!archoriginal.isEmpty())
		{
			...
			{
				//Leerá los datos y creará la tabla de variables
				...
				if(etiqueta.equals("id"))
				{
					...
					{
						if(!condicionactual.isEmpty()) //Si ese id se utilizará para condiciones
						{
							//Tiene elementos
						...
						{	
							if(tipodatoactual.equals("")) 
							{
								...
								for (int j = 0; j < TablaVariables.length; j++) 
								{
									...
									{
										//Busca la posición de la variable en la tabla
										posvariableactual=j;
										encontrado=true;
										break;
									}
				...
				if(etiqueta.equals("if"))
				{
					...
					{
						//Ese número de if ya llegó a usarse
				...
				if(etiqueta.equals("do"))
				{
					...
					{
						//Ese número de dowhile ya llegó a usarse
				...
				if(etiqueta.equals("begin"))
				{
					...
					{
						//Ese número de beginend ya llegó a usarse
				...
				if(etiqueta.equals("end"))
				{
					...
					try
					{
						...
						{//Si no encuentra un else, se terminó el ciclo if
							nivelif--;
						}
					}
				...
				//Los siguientes términos se agregarán a una expresión
				if(etiqueta.equals("+"))
				{
					if(igual)
					{
			...
			else
			{
				...
				/*else
				{
					System.out.println("Tipos de datos erróneos...");
				}*/
			}
		}
	}
